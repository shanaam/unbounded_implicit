...
rot_nocur$block_num <- sapply(rot_nocur$task_num, block_num)
...
nocur_summary_blocks <- rot_nocur %>%
  ...
  summarise(mean_devs = mean(angular_dev), 
            ...
            n = n()) %>%
  filter((exp == "stepwiseExp" & rotation_angle == -60) | (exp == "longAbruptExp" & (block_num == 1 | block_num == 4)))
...
data <- 
  nocur_summary_blocks %>%
  filter(rotation_angle == -60) %>%
  group_by(exp, block_num, stratuse) %>%
  summarise(mean_dev = mean(mean_devs), 
            ci = vector_confint(mean_devs)) %>%
  filter(block_num == 1 | block_num == 4)

data$dummy <- factor(c(1,1,2,2,3,3))
nocur_summary_blocks$dummy <- factor(nocur_summary_blocks$dummy)
  
p <- data %>%
  ggplot(aes(x = stratuse, y = mean_dev, colour = dummy)) +
  geom_beeswarm(data = nocur_summary_blocks, 
                aes(y = mean_devs, colour = dummy),
                size = 3, alpha = 0.6, groupOnX = TRUE, dodge.width = .4) +
  geom_linerange(aes(ymin = mean_dev - ci, ymax = mean_dev + ci),
                 lwd = 20, alpha = 0.4, position = position_dodge(width = .4)) +
  geom_point(size = 14, alpha = .6, position = position_dodge(width = .4)) +
  scale_x_discrete(name = "strategy use", labels = c('without strategy', 'with strategy')) +
  scale_y_continuous(limits = c(0, 50), breaks = c(0, 15, 30, 45, 60), name = "hand deviation (°)") +
  theme_minimal() +
  theme(text = element_text(size=40), 
        axis.text = element_text(size=40), 
        legend.text = element_text(size=48), 
        panel.grid.major.y = element_line(colour = "#CCCCCC")) + 
  scale_colour_manual(values=c("#6baed6", "#084594", "#d40000"), 
                       name= NULL,
                       breaks=c("1", "2", "3"),
                       labels=c( "abrupt experiment [block 1]",
                                 "abrupt experiment [block 4]",
                                 "stepwise experiment [block 4]")) +
  NULL

p


ggsave(p, height = 14, width = 14, device = "svg", filename = "data/noCur_plot_test.svg")
...
p <- ggplot(data = data[data$stratuse == 0, ], aes(block_num, mean_dev, colour = exp)) +
  ...
  scale_colour_manual(values=c("#084594", "#d40000"), 
                       name= NULL,
                       breaks=c("stepwiseExp", "longAbruptExp"),
                       labels=c( "stepwise", "abrupt"))
...
# Make plots for Learning Curves
...
p <- training_summary %>%
  ...
  scale_colour_manual(values=c( "#084594", "#d40000"), 
                       name= NULL,
                       breaks=c("longAbruptExp", "stepWiseExP"),
                       labels=c( "abrupt experiment", "stepwise experiment")) +
  theme(panel.grid.major.y = element_line(colour = "#CCCCCC")) +
  NULL
