...
nocur_summary <- rot_nocur %>%
  group_by(exp, ppt, rotation_angle, block_num, strat_use) %>%
...
```{r}
p <- rot_all %>% 
  ggplot(aes(trial_num_cont, angular_dev, colour = exp)) +
  geom_point(alpha = 0.03)+
  geom_point(data = rot_all_summary, aes(y = mean, shape = strat_use)) +
