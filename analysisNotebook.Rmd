...
# we will get a mean for each participant
...
# set colour
p <- p + scale_color_manual(values = c(cbp1[2],
                                  cbp1[2],
                                  cbp1[3],
                                  cbp1[3],
                                  cbp1[4]), labels = c("Abrupt Block 1", 
                                                       "Abrupt Block 4",
                                                       "Ramped Block 1",
                                                       "Ramped Block 4",
                                                       "Abrupt Block 1"))
p <- p + theme(panel.grid.major.y = element_line(colour = "#CCCCCC"),
  ...
  xlab(NULL) + ylab("reach aftereffects without strategy (°)")
p

ggsave(p, height = 9, width = 5, device = "png", filename = "data/paper_figs/implicit_aes.png")
```
...
ggsave(p, height = 9, width = 5, device = "png", filename = "data/paper_figs/explicit_aes.png")
```
...
rot_nocur_trial_summary <- rot_nocur %>%
  group_by(exp, block_num_detailed, trial_num_cont, strat_use) %>%
  summarise(mean = mean(angular_dev),
            sd = sd(angular_dev), 
            ci = vector_confint(angular_dev),
            n = n(), .groups = "drop")

rot_nocur_detailed_block_summary <-  rot_nocur %>%
  group_by(exp, ppt, block_num_detailed, strat_use) %>%
  summarise(angular_dev_temp = mean(angular_dev),
            sd = sd(angular_dev), 
            ci = vector_confint(angular_dev),
            n = n(), .groups = "drop") %>%
  group_by(exp, block_num_detailed, strat_use) %>%
...
for (strat_use_i in unique(rot_nocur_trial_summary$strat_use)){
  for (block_num_i in unique(rot_nocur_trial_summary$block_num_detailed)){
    for (exp_i in unique(rot_nocur_trial_summary$exp)){
      ...
      temp <- rot_nocur_trial_summary %>%
        filter(strat_use == strat_use_i, block_num_detailed == block_num_i, exp == exp_i)
      ...
      bool_arr <- rot_nocur_detailed_block_summary$strat_use == strat_use_i & 
                                         rot_nocur_detailed_block_summary$block_num_detailed == block_num_i &
      ...
      bool_arr <- rot_nocur_trial_summary$strat_use == strat_use_i & 
                                         rot_nocur_trial_summary$block_num_detailed == block_num_i &
                                          rot_nocur_trial_summary$exp == exp_i
      rot_nocur_trial_summary[bool_arr, ]$fitted_values <- array(lm_fit$fitted.values)
      rot_nocur_trial_summary[bool_arr, ]$short_trials <- temp$trial
    }
  }
}
```
...
p <- rot_nocur_trial_summary %>% 
  ggplot(aes(short_trials, mean, colour = exp, shape = strat_use)) +
  scale_shape_manual(values=c(20, 2))+
  # stat_summary(fun=mean, geom="point", size=3, color="red") +
  theme_minimal() +
  theme(panel.grid.major.y = element_line(colour = "#CCCCCC")) +
...
p <- p + 
  ...
  geom_point(aes( x = short_trials, y = mean), alpha = 0.5) +
...
p <- p +
  facet_grid(rows = vars(strat_use), cols = vars(block_num_detailed))

p

# ggsave(p, height = 9, width = 16, device = "svg", filename = "data/paper_figs/no_cur_regressions.svg")
```
```{r}
summary(lm.stepped_1.1)
```
