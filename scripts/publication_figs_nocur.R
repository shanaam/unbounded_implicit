...
make_decay_implicit_figure <- function() {
    ...
    # add theme changes and horizontal lines
    p <- p + theme_classic() +
    ...
    p <- p +
        ...
        geom_linerange(aes(
            ymin = group_slope_mean - slope_ci,
...
make_decay_withstrat_figure <- function() {
    # load in the DECAY data
    data <- load_bl_corrected("data/bl_corrected/bl_corrected_data.csv",
        type = "nocursor"
    )
...
make_decay_explicitstrat_figure <- function() {
    # load in the DECAY data
    data <- load_bl_corrected("data/bl_corrected/bl_corrected_data.csv",
        type = "nocursor"
    )
    ...
    data <- data %>%
        mutate(strat_use = ifelse(strat_use == "strat_0", "Without Strategy",
            ifelse(strat_use == "strat_1", "With Strategy",
                ifelse(strat_use == "strategy", "With - Without Strategy",
                    strat_use
                )
            )
        ))
    ...
    data <- data %>%
        mutate(strat_use = factor(strat_use,
            levels = c("Without Strategy", "With Strategy", "With - Without Strategy")
        )) %>%
        filter(strat_use == "With - Without Strategy") %>%
        unite(exp_block, block_num, exp)
...
# save the decay line figure
ggsave(make_decay_figure(),
    height = 6, width = 3.5, device = "pdf",
    filename = "data/paper_figs/decay_aes.pdf"
)

# the 3 decay group figures
ggsave(make_decay_implicit_figure(),
    height = 2, width = 3.5, device = "pdf",
    filename = "data/paper_figs/decay_ind_withoutstrat_aes.pdf"
)

ggsave(make_decay_withstrat_figure(),
    height = 2, width = 3.5, device = "pdf",
    filename = "data/paper_figs/decay_ind_withstrat_aes.pdf"
)

ggsave(make_decay_explicitstrat_figure(),
    height = 2, width = 3.5, device = "pdf",
    filename = "data/paper_figs/decay_ind_strategy_aes.pdf"
)
